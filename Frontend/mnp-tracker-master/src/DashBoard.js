...
import {
  FaChartBar, FaExchangeAlt, FaKey, FaBell, FaMoon, FaSun, FaUserCircle
} from "react-icons/fa";

import {
  Drawer, IconButton, List, ListItem, ListItemText, Menu, MenuItem,
  useMediaQuery, useTheme, Box, AppBar, Toolbar, Typography, Divider
} from "@mui/material";
...
const userName = location.state?.phoneNumber || "User";
  ...
  const drawerContent = (
    <>
      ...
      <List>
        <ListItem button onClick={() => { navigate("/checking-status"); if (!isDesktop) setMobileDrawerOpen(false); }}>
          <FaChartBar style={{ marginRight: 8 }} />
          <ListItemText primary="Check Porting Status" />
        </ListItem>
        <ListItem button onClick={() => { navigate("/request-porting"); if (!isDesktop) setMobileDrawerOpen(false); }}>
          <FaExchangeAlt style={{ marginRight: 8 }} />
          <ListItemText primary="Request New Porting" />
        </ListItem>
        <ListItem button onClick={() => { navigate("/upc-info"); if (!isDesktop) setMobileDrawerOpen(false); }}>
  ...
  return (
    <Box sx={{ display: "flex" }}>
      <AppBar position="fixed" sx={{ zIndex: theme.zIndex.drawer + 1, backgroundColor: "#223354" }}>
        <Toolbar>
          {!isDesktop && (
            <IconButton
              color="inherit"
    ...
    <Box sx={{ display: "flex", alignItems: "center", mr: 2 }}>
      ...
      />
          ...
          {/* MNP Tracker title and the rest... */}
    <Typography variant="h5" noWrap component="div" sx={{ flexGrow: 1, color: "#35cefe", fontWeight: "bold" }}>
      MNP Tracker
    </Typography>
  ...
  Hi, {userName}
            ...
            >
<MenuItem component={RouterLink} to="/settings" onClick={handleProfileClose}>Settings</MenuItem>
<MenuItem component={RouterLink} to="/" onClick={handleProfileClose}>logout</MenuItem>

</Menu>  {/* Add this closing tag */}
...
color: "#fff"
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3, mt: 8 }}>
        <div className="dashboard-welcome-card">
          ...
          <p>
            Seamlessly manage requests, check statuses, and access UPC codes <b>all in one secure place.</b>
          </p>
        </div>
